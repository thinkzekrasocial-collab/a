...
interface DashboardData {
  ...
  machine_availability: { id: number; machine_code: string; machine_name: string; status: string; availability_status: string; model: string | null; machine_type_name: string; unit_name: string; floor_name: string }[];
}
  ...
  return <div>
    ...
    <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
      ...
      <Card title="Machines Availability" actions={perms.includes("machine.view") ? <Link href="/machines" className="text-xs font-medium text-teal-700 hover:underline">Open machines →</Link> : null}>
        {data.machine_availability.length === 0 ? <EmptyState icon="⚙️" title="No machines yet" message="Add machines to see their availability." /> : <Table headers={["Machine", "Location", "Availability", "Status"]}>{data.machine_availability.map((m) => <tr key={m.id}><Td><span className="font-medium text-slate-800">{m.machine_name}</span><span className="ml-1.5 text-xs text-slate-400">{m.machine_code}</span></Td><Td>{m.unit_name} · {m.floor_name}</Td><Td><Badge tone={m.availability_status === "Available" ? "green" : "red"}>{m.availability_status}</Badge></Td><Td><Badge tone={m.status === "Running" || m.status === "Idle" ? "green" : "amber"}>{m.status}</Badge></Td></tr>)}</Table>}
      </Card>
    </div>
