      ...
      <div className="overflow-hidden rounded-2xl border border-[#dedcd6] bg-white shadow-[0_2px_8px_rgba(38,36,30,0.04)]">
        <div className="overflow-x-auto">
          <table className="min-w-[1080px] w-full border-collapse text-[12px]">
            ...
            <tbody>{rows.map((row, index) => { const calc = calculations[index]; return <tr key={row.id} className="border-b border-[#efeee9] last:border-b-0"><td className="px-4 py-2"><div className="flex items-center gap-2"><Input aria-label={`Date row ${index + 1}`} type="date" value={row.date} disabled={!canEdit} onChange={(event) => updateRow(row.id, "date", event.target.value)} className="sheet-input min-w-[135px] border-0 bg-transparent px-0 shadow-none" /><span className="text-[#77756f]">▣</span></div></td><td className="px-3 py-2"><Input aria-label={`Balance row ${index + 1}`} type="text" inputMode="decimal" value={row.balance} disabled={!canEdit} onChange={(event) => updateRow(row.id, "balance", event.target.value)} placeholder={index ? "prev: 0" : ""} className="sheet-input" /></td><td className="px-3 py-2"><Input aria-label={`Out row ${index + 1}`} type="text" inputMode="decimal" value={row.out} disabled={!canEdit} onChange={(event) => updateRow(row.id, "out", event.target.value)} className="sheet-input" /></td><td className="px-3 py-2"><Input aria-label={`Given row ${index + 1}`} type="text" inputMode="decimal" value={row.given} disabled={!canEdit} onChange={(event) => updateRow(row.id, "given", event.target.value)} className="sheet-input" /></td><td className="px-3 py-2"><div className="sheet-output">{fmtNum(calc.inBalance)}</div></td><td className="px-3 py-2"><div className="sheet-output text-[#42413d]">{fmtNum(calc.total)} <span className="text-[10px] text-[#8f8d86]">{index ? `0 - ${fmtNum(calc.out)}` : "open-out"}</span></div></td><td className="px-3 py-2"><div className="sheet-output font-semibold text-[#242424]">{fmtNum(calc.remained)}</div></td></tr>; })}</tbody>
          </table>
        </div>
